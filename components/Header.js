            ...
            {nav.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={`nav-link-underline pointer-events-auto inline-block whitespace-nowrap text-[15px] font-semibold no-underline transition-colors hover:text-accent ${
                    scrolled ? "pointer-events-none" : ""
                  } ${active ? "is-active text-accent" : "text-foreground"}`}
                >
                  {item.label}
                </Link>
              );
